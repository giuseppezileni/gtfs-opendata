...
const downloadFiles = task => {
  ...
  var path = `${task.downloadDir}/${task.agency_key}-gtfs.zip`
...
const importFiles = task => {
  ...
  return Promise.each(filenames, filename => {
    ...
    */
    ...
    const filepath = path.join(task.downloadDir + '/' + task.dir, `${filename.fileNameBase}.txt`);
...
module.exports = async (config, url, mongoUrl) => {

  console.log('Import GTFS Data: ' + JSON.stringify(config));
  ...
  const db = await MongoClient.connect(mongoUrl, mongoOptions);
  ...
  for (const agency of config.agencies) {
    ...
    const task = {
      ...
      agency_url: url + agency.file,
      path: path.resolve('./') + path.basename(url + agency.file),
